...
!!! Note
    ...
    about different resources when it comes to troubleshooting EDB Postgres for Kubernetes.
...
Get logs from all pods in EDB Postgres for Kubernetes operator Deployment
...
PostgreSQL Image:   quay.io/enterprisedb/postgresql:16.4-3
...
!!! Note
    ...
    For more information, please see [`cnp` plugin](kubectl-plugin.md) documentation.
...
```shell
  Image Name:    quay.io/enterprisedb/postgresql:16.4-3
```
...
!!! Important
    Backup labelling has been introduced in version 1.10.0 of EDB Postgres for Kubernetes.
...
EDB Postgres for Kubernetes requires basic networking and connectivity in place.
You can find more information in the [networking](networking.md) section.

If installing EDB Postgres for Kubernetes in an existing environment, there might be
